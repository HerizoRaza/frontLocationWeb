import { LayoutDashboard, Users , LogIn ,Car ,Settings, Handshake , LogOut } from 'lucide-react'

// On définit les liens de navigation dans une constante (bonne pratique)
// → Facile à modifier sans toucher au JSX
const navItems = [
  { icon: LayoutDashboard, label: 'Dashboard', href: '#' },
  { icon: LogIn,           label: 'Authetification', href: '#' },
  { icon: Users ,       label: 'Chauffeur', href: '#' },
  { icon: Car ,       label: 'Voiture', href: '#' },
  { icon: Handshake ,       label: 'Partenaire', href: '#' },
...
export default function Sidebar() {
  return (
    <aside className="w-64 min-h-screen bg-(--color-dark) flex flex-col">
      ...
      {/* Logo */}
      <div className="h-16 flex items-center px-6 border-r-2 border-(--color-primary) bg-(--color-dark)">
        <span className="text-white font-bold text-xl">⚡TSINJOLAVITRA</span>
      </div>

      {/* Navigation */}
      <nav className="flex-1 px-4 py-6 border-r-2 border-(--color-primary) space-y-1">
        {navItems.map((item) => (
          <a
            key={item.label}
            href={item.href}
      ...
      <div className="px-4 py-4 border-t border-r-2 border-(--color-primary)" >
